...
num_swipes = 4
epss = np.geomspace(0.6, 1e-2, 2*num_swipes).tolist()
plt.plot(epss)
...
layer.tensor_network.accumulating_swipe(xinp_train, y_train, bf, batch_size=4096, delta=3.0, lr=1.0, convergence_criterion=convergence_criterion, orthonormalize=False, method='ridge_exact', eps=epss, verbose=2, num_swipes=num_swipes, skip_second=False, direction='l2r')
#%%
for i in range(20):
    layer.grow_cart(r, CB)
    layer.cuda()
with torch.inference_mode():
    y_pred = layer(xinp_train[:64])
    w = 1/y_pred.std().item()
...
for i in range(20):
    layer.grow_cart(r, CB)
    layer.cuda()
    if not layer.tensor_network.accumulating_swipe(xinp_train, y_train, bf, batch_size=4096, delta=3.0, lr=1.0, convergence_criterion=convergence_criterion, orthonormalize=False, method='ridge_exact', eps=epss, verbose=2, num_swipes=num_swipes, skip_second=False, direction='r2l'):
        print("Got to step", i+1+N)
        break
#%%
from tensor.utils import visualize_tensornetwork
visualize_tensornetwork(layer.tensor_network)
